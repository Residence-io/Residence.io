...
export const environmentSchema = z
  .object({
    ...
    // Phase S2  Supabase Auth (all optional; gated by FEATURE_SUPABASE_AUTH)
  ...
  .superRefine((environment, context) => {
    ...
    if (
      environment.NODE_ENV === 'production' &&
      environment.PAYMENT_PROVIDER_MODE === 'sandbox'
    ) {
      context.addIssue({
        code: 'custom',
        path: ['PAYMENT_PROVIDER_MODE'],
        message: 'The payment sandbox provider cannot run in production.',
      });
    }
    if (
      environment.PAYMENT_PROVIDER_MODE === 'sandbox' &&
      !environment.PAYMENT_SANDBOX_SECRET
    ) {
      context.addIssue({
        code: 'custom',
        path: ['PAYMENT_SANDBOX_SECRET'],
        message: 'A sandbox callback secret is required in sandbox mode.',
      });
    }
    if (
      environment.NODE_ENV === 'production' &&
      environment.NOTIFICATION_PROVIDER_MODE === 'sandbox'
    ) {
      context.addIssue({
        code: 'custom',
        path: ['NOTIFICATION_PROVIDER_MODE'],
        message: 'The notification sandbox provider cannot run in production.',
      });
    }
    if (
      environment.NODE_ENV === 'production' &&
      environment.RESIDENCE_SEED_ENABLED
    ) {
      context.addIssue({
        code: 'custom',
        path: ['RESIDENCE_SEED_ENABLED'],
        message: 'Development seeding is disabled in production.',
      });
    }
    if (
      environment.NODE_ENV === 'production' &&
      environment.PRIVATE_STORAGE_ROOT.startsWith('apps/web/public')
    ) {
      context.addIssue({
        code: 'custom',
        path: ['PRIVATE_STORAGE_ROOT'],
        message: 'Private storage cannot be placed under public web assets.',
      });
    }
    if (environment.NODE_ENV === 'production') {
      for (const [name, value] of [
        ['SESSION_SECRET', environment.SESSION_SECRET],
        ['IDENTITY_DATA_KEY', environment.IDENTITY_DATA_KEY],
        [
          'NOTIFICATION_CALLBACK_SECRET',
          environment.NOTIFICATION_CALLBACK_SECRET,
        ],
        ['PAYMENT_SANDBOX_SECRET', environment.PAYMENT_SANDBOX_SECRET],
      ] as const) {
        if (value && unsafeProductionValue.test(value)) {
          context.addIssue({
            code: 'custom',
            path: [name],
            message: `${name} contains a known placeholder value.`,
          });
        }
      }
    }
